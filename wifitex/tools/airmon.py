...
class Airmon(Dependency):
    ...
    @staticmethod
    def _native_start(iface_name, before_monitors=None):
        '''
        Attempt to enable monitor mode using native iw/iwconfig commands before
        falling back to airmon-ng.
        Returns the interface name if successful, otherwise None.
        '''
        if before_monitors is None:
            before_monitors = set(Iwconfig.get_interfaces(mode='Monitor'))
        ...
        if Process.exists('iw'):
            try:
                iw_proc = Process(['iw', 'dev', iface_name, 'set', 'type', 'monitor'])
                iw_proc.wait()
                native_success = (iw_proc.poll() == 0)
            except Exception:
                native_success = False

        if not native_success:
            try:
                native_success = (Iwconfig.mode(iface_name, 'monitor') == 0)
            except Exception:
                native_success = False
        ...
        monitors_after = set(Iwconfig.get_interfaces(mode='Monitor'))
        if iface_name in monitors_after:
            return iface_name

        new_monitors = list(monitors_after - (before_monitors or set()))
        if len(new_monitors) == 1:
            return new_monitors[0]
    ...
    def start(iface):
        ...
        before_monitors = set(Iwconfig.get_interfaces(mode='Monitor'))
        native_iface = Airmon._native_start(iface_name, before_monitors=before_monitors)
        if native_iface:
        ...
        after_monitors = set(monitor_interfaces)

        if enabled_iface is None:
            new_monitors = list(after_monitors - before_monitors)
            if len(new_monitors) == 1:
                enabled_iface = new_monitors[0]
    ...
    def _parse_airmon_start(airmon_output):
        ...
        enabled_re = re.compile(
            r'.*\(mac80211 monitor mode (?:vif )?(?:already )?enabled (?:for [^\s]+ )?on (?:\[[^\]]+\])?([^\s\)]+)\).*',
            re.IGNORECASE
        )
        enabled_alt_re = re.compile(
            r'.*\(monitor mode (?:vif )?(?:already )?enabled (?:for [^\s]+ )?on (?:\[[^\]]+\])?([^\s\)]+)\).*',
            re.IGNORECASE
        )

        for line in (airmon_output or '').split('\n'):
            matches = enabled_re.match(line)
            if matches:
                return matches.group(1)
            matches = enabled_alt_re.match(line)
            if matches:
                return matches.group(1)
    ...
    def terminate_conflicting_processes():
        ...
        if not Configuration.kill_conflicting_processes:
            ...
            Color.pl('{!} {O}If you have problems: {R}kill -9 PID{O} or re-run wifitex with {R}--kill{O}){W}')
            return
        ...
        for pid, pname in pid_pnames:
            if pname == 'NetworkManager':
                ...
                if Process.exists('systemctl'):
                    stop_commands.append(['systemctl', 'stop', 'NetworkManager'])
                if Process.exists('service'):
                    stop_commands.append(['service', 'network-manager', 'stop'])

                stopped = False
                for cmd in stop_commands:
                    Color.pl('{!} {O}stopping network-manager ({R}%s{O})' % ' '.join(cmd))
                    proc = Process(cmd)
                    (out, err) = proc.get_output()
                    code = proc.poll()

                    if code == 0:
                        Color.pl('{+} {G}Stopped NetworkManager using %s{W}' % ' '.join(cmd))
                        Airmon.killed_network_manager = True
                        stopped = True
                        break

                    if out is not None and out.strip():
                        Color.pl('{!} {O}STDOUT> %s{W}' % out.strip())
                    if err is not None and err.strip():
                        Color.pl('{!} {O}STDERR> %s{W}' % err.strip())
                    Color.pl('{!} {R}Failed to stop NetworkManager using {O}%s{R} (exit code %s){W}' % (' '.join(cmd), code))

                if stopped:
                    continue

                Color.pl('{!} {R}Falling back to terminating NetworkManager process {O}(PID {R}%s{O}){W}' % pid)
                try:
