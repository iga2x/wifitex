...
class Aircrack(Dependency):
    ...
    def __init__(self, ivs_file=None):
        ...
        if isinstance(ivs_file, str):
            ivs_file = [ivs_file]
        elif ivs_file is None:
            ivs_file = []
    ...
    def crack_handshake(handshake, show_command=False, wordlist=None):
        ...
        if not wordlist_path:
            raise ValueError('No wordlist specified for aircrack-ng handshake attack')
        ...
        eta_str = 'unknown'
        current_key = ''
        stdout_stream = crack_proc.pid.stdout
        if stdout_stream is None:
            raise RuntimeError('aircrack-ng process did not capture stdout')
