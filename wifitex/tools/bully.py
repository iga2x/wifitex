...
class Bully(Attack, Dependency):
    ...
    def _run(self, airodump):
        bully_proc = self.bully_proc
        if bully_proc is None:
            self.pattack('{R}Failed: {O}Bully process not running{W}', newline=True)
            return
    ...
    def parse_line_thread(self):
        if not self.bully_proc:
            return

        stdout_stream = getattr(self.bully_proc.pid, 'stdout', None)
        if stdout_stream is None:
            if Configuration.verbose > 0:
                Color.pe('{R}[bully] stdout pipe is not available; cannot parse output{W}')
            return
    ...
    def stop(self):
        if self.bully_proc and self.bully_proc.poll() is None:
