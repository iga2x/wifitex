...
class Airodump(Dependency):
    ...
    def __init__(self, interface=None, channel=None, encryption=None,\
        ...
        if self.band is None:
            ...
            if Configuration.five_ghz:
                band_parts.append('a')
            if getattr(Configuration, 'six_ghz', False):
                band_parts.append('6')
            if band_parts:
                # Deduplicate while maintaining order
                band_string = ''
                for token in ''.join(band_parts):
                    if token not in band_string:
                        band_string += token
                self.band = band_string or None
    ...
    def filter_targets(targets, skip_wps=False):
        ...
        # If no encryption filter configured, keep all targets (matching CLI behaviour)
        if not Configuration.encryption_filter:
            result = [t for t in targets if not (Configuration.clients_only and len(t.clients) == 0)]
        else:
            result = []
