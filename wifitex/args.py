...
class Arguments(object):
    ''' Holds arguments used by Wifitex '''
    ...
    def _add_global_args(self, glob):
        ...
        glob.add_argument('--6ghz',
            action='store_true',
            dest='six_ghz',
            help=self._verbose('Include 6Ghz channels (default: {G}off{W})'))
    ...
    def _add_wps_args(self, wps):
        ...
        wps.add_argument('--wps-pin-precheck-timeout',
            action='store',
            dest='wps_pin_precheck_timeout',
            metavar='[sec]',
            type=int,
            help=self._verbose('Seconds to wait for each suggested PIN attempt (0 = wait forever, default: {G}%d{W})' %
                               self.config.wps_pin_precheck_timeout))
        wps.add_argument('-wpsppt', help=argparse.SUPPRESS, action='store',
                dest='wps_pin_precheck_timeout', type=int)
