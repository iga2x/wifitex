An idea from Sandman: Include "Evil Twin" attack in Wifitex.

This page tracks the requirements for such a feature.

**NOTE: As of the current implementation, Evil Twin functionality has been superseded by the KARMA attack, which provides enhanced Evil Twin capabilities with PNL (Preferred Network List) capture. The KARMA attack includes all Evil Twin functionality plus dynamic SSID spoofing based on victim preferences, making it more effective than a basic Evil Twin attack. This document is kept for reference.**
...
Below are all of the requirements/components that Wifitex would need for this feature.
...
I think MDK is a better tool for this job, but Wifitex already requires the `aircrack` suite, so we should support both.
