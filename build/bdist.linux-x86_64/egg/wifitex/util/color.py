...
class Color(object):
    ...
    def clear_entire_line():
        ...
        try:
            ...
            stty_output = os.popen('stty size', 'r').read().strip()
            if stty_output and ' ' in stty_output:
                (rows, columns) = stty_output.split()
                Color.p('\r' + (' ' * int(columns)) + '\r')
            else:
                # Fallback to default terminal width
                Color.p('\r' + (' ' * 80) + '\r')
        except (ValueError, OSError):
            # Fallback for non-interactive environments or when stty fails
            Color.p('\r' + (' ' * 80) + '\r')
    ...
    def pattack(attack_type, target, attack_name, progress):
        ...
        Includes attack type (WPA/WPS), target ESSID & power, attack type, and progress.
        ESSID (Pwr) Attack_Type: Progress
        e.g.: Router2G (23db) WPA handshake attack: 102 IVs
        '''
