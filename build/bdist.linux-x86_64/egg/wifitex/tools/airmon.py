...
from ..util.input import input as raw_input
...
# Resolve base exception class for this module without alias reassignments
if TYPE_CHECKING:
    class _BaseInterfaceError(Exception):
        pass
else:
    try:
        from ..gui.error_handler import InterfaceError as _BaseInterfaceError
    except Exception:
        class _BaseInterfaceError(Exception):
            pass

class AirmonInterfaceError(_BaseInterfaceError):
    pass
...
class Airmon(Dependency):
    ...
    def get_interfaces():
        ...
        interfaces = []
        p = Process('airmon-ng')
        stdout_output = p.stdout()
        if stdout_output is None:
            stdout_output = ''
        elif isinstance(stdout_output, bytes):
    ...
    def start_bad_driver(iface):
        ...
        # Use dynamic sysfs path for interface type detection
        sysfs_net_path = '/sys/class/net'
        iface_type_path = os.path.join(sysfs_net_path, iface, 'type')
    ...
    def start(iface):
        ...
        if len(monitor_interfaces) == 0:
            Color.pl('{R}failed{W}')
            raise AirmonInterfaceError('Cannot find any interfaces in Mode:Monitor')

        # Assert that the interface enabled by airmon-ng is in monitor mode
        if enabled_iface not in monitor_interfaces:
            Color.pl('{R}failed{W}')
            raise AirmonInterfaceError('Cannot find %s with Mode:Monitor' % enabled_iface)
    ...
    def ask():
        ...
        if count == 0:
            ...
            raise AirmonInterfaceError('airmon-ng did not find any wireless interfaces')
    ...
    def terminate_conflicting_processes():
        ...
        # Handle case where airmon_output might be None or bytes
        if airmon_output is None:
            airmon_output = ''
        elif isinstance(airmon_output, bytes):
            airmon_output = airmon_output.decode('utf-8')
