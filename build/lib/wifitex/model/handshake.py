...
class Handshake(object):
    ...
    def divine_bssid_and_essid(self):
        ...
        # We can get BSSID from the .cap filename if Wifitex captured it.
    ...
    def cowpatty_handshakes(self):
        ...
        if stdout_output is None:
            return []
        # Ensure we have a string for splitting
        stdout_str = str(stdout_output)
    ...
    def aircrack_handshakes(self):
        ...
        # Check for various indicators that a handshake exists
        if stdout:
            stdout_lower = str(stdout).lower()
            if 'passphrase not in dictionary' in stdout_lower or \
               ('1 potential targets' in stdout_lower and 'opening' in stdout_lower):
    ...
    def check():
        ...
        for capfile in capfiles:
            ...
            if capfile and not os.path.exists(capfile):
