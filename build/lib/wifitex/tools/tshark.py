...
class Tshark(Dependency):
    ...
    def _build_target_client_handshake_map(output, bssid=None):
        ...
        for line in output.split('\n'):
            ...
            if src is None or index is None or total is None: continue # Skip
            ...
            if bssid is not None and target is not None and bssid.lower() != target.lower():
                # We know the BSSID and this msg was not for the target
                continue

            if target is None or client is None:
                continue  # Skip if we don't have valid MAC addresses
    ...
    def bssid_essid_pairs(capfile, bssid):
        ...
        for line in output.split('\n'):
            ...
            mac_regex = ('[a-zA-Z0-9]{2}:' * 6)[:-1]
            # Updated regex to match tshark output format: "packet_num timestamp src → dst ... SSID="essid""
            match = re.search(r'\s*\d+\s+[\d.]+\s+(%s)\s+→\s+(%s).*SSID="([^"]*)"' % (mac_regex, mac_regex), line)
            if match is None:
...
if __name__ == '__main__':
    ...
    import os
    test_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'tests', 'files', 'contains_wps_network.cap')
