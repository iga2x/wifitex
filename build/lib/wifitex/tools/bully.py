...
class Bully(Attack, Dependency):
    ...
    def __init__(self, target, pixie_dust=True):
        ...
        self.cmd = Process.build_wps_command('bully', target, Configuration.interface, pixie_dust)
        self.bully_proc: Optional[Process] = None
    ...
    def run(self):
        with Airodump(channel=self.target.channel,
            ...
            t.daemon = True  # Consistent daemon setting
            t.start()
    ...
    def _run(self, airodump):
        while self.bully_proc and self.bully_proc.poll() is None:
            ...
            if self.pixie_dust:
                ...
                # PIN attack timeout handling
                pin_timeout = getattr(Configuration, 'wps_pin_timeout', Configuration.wps_pixie_timeout)
                if self.running_time() > pin_timeout:
                    self.pattack('{R}Failed: {O}PIN attack timeout after %d seconds{W}' % (
                        pin_timeout), newline=True)
                    self.stop()
                    return

                # Check if timeout threshold was breached for PIN attacks
                if self.total_timeouts >= Configuration.wps_timeout_threshold:
                    self.pattack('{R}Failed: {O}More than %d Timeouts{W}' % (
                        Configuration.wps_timeout_threshold), newline=True)
                    self.stop()
                ...
                # Check if WPSFail threshold was breached for PIN attacks
                if self.total_failures >= Configuration.wps_fail_threshold:
                    self.pattack('{R}Failed: {O}More than %d WPSFails{W}' % (
                        Configuration.wps_fail_threshold), newline=True)
                    self.stop()
    ...
    def parse_line_thread(self):
        if not self.bully_proc or not self.bully_proc.pid:
            return
        
        # Access the underlying Popen object's stdout
        stdout = self.bully_proc.pid.stdout
        if stdout is None:
            return
    ...
    def parse_state(self, line):
        ...
        # [Pixie-Dust] Pixiewps failed
        re_pixiewps_failed = re.search(r".*\[Pixie-Dust\].*pixiewps.*failed", line, re.IGNORECASE)
        if re_pixiewps_failed:
            state = '{R}Failed: {O}Pixiewps failed to crack the PIN{W}'
    ...
    def stop(self):
        if hasattr(self, 'bully_proc') and self.bully_proc and self.bully_proc.poll() is None:
            ...
            time.sleep(1)
            if self.bully_proc.poll() is None:
                # Force terminate if still running
                self.bully_proc.interrupt(wait_time=0.1)
...
if __name__ == '__main__':
    ...
    # Use dynamic interface detection
    from ..gui.utils import SystemUtils
    interfaces = SystemUtils.get_wireless_interfaces()
    Configuration.interface = interfaces[0] if interfaces else None
