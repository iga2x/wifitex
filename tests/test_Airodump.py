...
class TestAirodump(unittest.TestCase):
    ''' Test suite for Wifitex's interaction with the Airodump tool '''
    ...
    def getFile(self, filename):
        ...
        import os, inspect
        source_file = inspect.getsourcefile(self.getFile)
        if source_file is None:
            raise RuntimeError("Could not determine source file for test")
        this_file = os.path.abspath(source_file)
